(function(annyang) {
    ...
    function SubwayService($http) {
        ...
        service.init = function(station){
            return $http.get("http://openAPI.seoul.go.kr:8088/"+config.subway.key1+"/xml/SearchSTNBySubwayLineService/1/20/"+station+"/").
            then(function(response) {
                var x2js = new X2JS();
                var jsonData = x2js.xml_str2json(response.data);
                if(jsonData.SearchSTNBySubwayLineService.row){
                    service.stationrow = jsonData.SearchSTNBySubwayLineService.row;
                }
            });
        };
        ...
        service.getArriveTime = function(linenumber,updown) {
            ...
            return $http.get("http://openAPI.seoul.go.kr:8088/"+config.subway.key2+"/xml/SearchArrivalInfoByIDService/1/5/"+stationcode+"/"+updowncode+"/"+weekcode+"/").
            then(function(response) {
                var x2js = new X2JS();
                var jsonData = x2js.xml_str2json(response.data);
